...
class modmail(commands.Cog):
    ...
    async def modmail_nieuw_ticket(self, inter, user: disnake.User):
        ...
        thread = await channel.create_thread(name=f"{str(user.display_name)}&&MM", reason=f"ModMail for {user.mention}",
                                             type=disnake.ChannelType.text, message=None)
    ...
    async def first_contact(self, message):
        ...
        if message.attachments:
            embed = disnake.Embed(title="Contact ADT&G beheer", description="Wil je dit versturen?",
    ...
    async def create_contact_with_staff(self, message):
        ...
        channel = self.bot.get_channel(env_variable.MODMAIL_ID)
        thread_message = await channel.send(f"Nieuwe ModMail ticket van {message.author.display_name}!")

        thread = await channel.create_thread(name=f"{str(message.author.name)}&&MM",
                                             reason=f"ModMail for {message.author.name}", type=None, message=thread_message)
        ...
        if message.attachments:
            embed = disnake.Embed(title="Nieuwe ticket", description=f"Van user: {message.author.name}",
    ...
    async def accept_or_deny(self, message, type, by_user):
        if type == True:
            embed = disnake.Embed(title="Contact ADT&G beheer",
                                  description=f"Verzoek geaccepteerd door `{by_user}`! Het kan even duren voor je antwoord krijgt.",
                                  color=disnake.Color.green())
        else:
            embed = disnake.Embed(title="Contact ADT&G beheer",
                                  description=f"Verzoek geweigerd door `{by_user}`! Probeer het anders opnieuw met een betere vraag.",
                                  color=disnake.Color.red())
    ...
    async def send_admin_response_to_ticket_maker(self, message, member):
        embed = disnake.Embed(title=f"Contact ADT&G antwoord", description=f"Door beheerder: `{message.author.name}`",
                              color=disnake.Color.green())
        embed.add_field(name=f"Antwoord beheer:", value=str(message.clean_content), inline=False)
        embed.add_field(name=f"Info:",
                        value="Als je vraag niet beantwoord is, dan kun je hieronder gewoon terugtypen! (In 1 bericht)",
                        inline=False)
    ...
    async def close_ticket_and_thread(self, message, member):
        class Confirm(disnake.ui.View):
            ...
            async def confirm(self_inside_button, button: disnake.ui.Button, inter: disnake.MessageInteraction):
                ...
                embed = disnake.Embed(title="Contact ADT&G ticket gesloten!",
                                      description=f"Door: `{message.author.name}`", color=disnake.Color.red())
                await member.send(embed=embed)
                self_inside_button.stop()
        ...
        if message.attachments:
            embed = disnake.Embed(title="Je hebt dit gestuurd:", description=str(message.clean_content),
                                  color=disnake.Color.green())
            embed.set_image(url=message.attachments[0].url)
            await message.channel.send(embed=embed, view=view)
    ...
    async def send_user_response(self, message, thread_to_send):
        class Confirm(disnake.ui.View):
            ...
            async def confirm(self_inside_button, button: disnake.ui.Button, inter: disnake.MessageInteraction):
                await inter.response.send_message("Ik heb de ticket gesloten!")
                embed = disnake.Embed(title="Contact ADT&G ticket gesloten!",
                                      description=f"Door: `{inter.author.name}`", color=disnake.Color.red())
        ...
        if message.attachments:
            embed = disnake.Embed(title="User reageerde met de afbeelding:", description=str(message.clean_content),
                                  color=disnake.Color.red())
            embed.set_image(url=message.attachments[0].url)
            await thread_to_send.send(embed=embed, view=view)
            return

        embed = disnake.Embed(title="User reageerde:", description=str(message.clean_content),
                              color=disnake.Color.red())
        await thread_to_send.send(embed=embed, view=view)
